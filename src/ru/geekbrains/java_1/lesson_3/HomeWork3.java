...
public class HomeWork3 {
    ...
    private static void initMap(int size_x, int size_y, int win_length){
        field_size_y = size_y;
        field_size_x = size_x;
        win_len = win_length;
    ...
    private static boolean checkWin(char c){
        for (int i = 0; i < field_size_x; i++) {
            for (int j = 0; j < field_size_y; j++) {
                if (checkLine(i, j, 1, 0, win_len, c)) return true;
                if (checkLine(i, j, 1, 1, win_len, c)) return true;
                if (checkLine(i, j, 0, 1, win_len, c)) return true;
                if (checkLine(i, j, 1, -1, win_len, c)) return true;
            }
        }
        return false;
